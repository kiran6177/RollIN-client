              ...
              <Toaster richColors />
                ...
                className='bg-black text-white w-[80%] sm:w-[65%] lg:w-[45%] xl:w-[35%] border-2 border-[#f6ae2d] rounded-md p-8 flex flex-col gap-16 justify-center items-center fixed min-h-[70vh] '>
                      <IoIosClose onClick={()=>set(false)} className="absolute cursor-pointer right-3 top-3 w-[2rem] h-[2rem]"/>
                        <h1 className='font-medium text-lg tracking-wider text-center'>Enroll Movie</h1>
                        <div className='w-[100%] flex flex-col items-center gap-4 '>
                        <h3 className='my-2 text-xl font-semibold tracking-widest w-[90%] overflow-hidden text-ellipsis whitespace-nowrap text-center text-[#f6ae2d]'>{movie?.title}</h3>
                        <h3 className='my-2 text-lg font-semibold tracking-widest w-[90%] overflow-hidden text-ellipsis whitespace-nowrap text-center text-white'>{movie?.release_date}</h3>
                                ...
                                <div className=' overflow-scroll scrollbar-none flex flex-col justify-start text-black w-[90%] h-[7rem] '>
                                    ...
                                    theatreScreenData.map((screen,i)=>{
                                        ...
                                        <div key={i} onClick={()=>{setScreenId(screen._id);setScreenSelect(false)}} className='min-h-[3rem] flex items-center bg-white rounded-sm  ml-[1.5px] mt-[1px] border-black w-[100%]'>
                                            <p className='mx-5 w-[100%]  text-ellipsis overflow-hidden'>{screen.name}</p>
                                        </div>
                                        )
                                    })
                                    }
                        ...
                        <h3 className='my-2 text-xl font-semibold tracking-widest w-[90%] overflow-hidden text-ellipsis whitespace-nowrap text-center text-[#f6ae2d]'>{screenName}</h3>
                            }
                        </div>
